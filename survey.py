...
class MultipleChoiceQuestion(Question):
    ...
    def __str__(self) -> str:
        ...
        #     options += option
        # return 'Question: ' + self.text + 'Options: ' + options
        return 'Question: {0} Options: {1}'.format(self.text, self._options)
    ...
    def validate_answer(self, answer: Answer) -> bool:
        ...
        if answer is None:
            return False
        ans = answer.content
        return isinstance(ans, str) and ans in self._options
...
class NumericQuestion(Question):
    ...
    def __str__(self) -> str:
        ...
        You can choose the precise format of this string.
        """
        # return 'Question: ' + self.text + '\n Hint: Any integer between ' \
        #        + str(self._min) + 'and ' \
        #        + str(self._max) + 'inclusive'
        return 'Question: {0} Hint: Any integer between {1} and {2} inclusive \
        '.format(self.text, self._min, self._max)
    ...
    def validate_answer(self, answer: Answer) -> bool:
        ...
        if answer is None:
            return False
        ans = answer.content
        return isinstance(ans, int) and self._min <= ans <= self._max
    ...
    def get_similarity(self, answer1: Answer, answer2: Answer) -> float:
        ...
        <answer1> and <answer2> are both valid answers to this question
        """
        ans1 = answer1.content
        ans2 = answer2.content
        diff = abs(ans1 - ans2)
        default_diff = self._max - self._min
        return 1.0 - (diff / default_diff)
...
class YesNoQuestion(Question):
    ...
    def __str__(self) -> str:
        ...
        """
        # return 'Question: ' + self.text + '\nOptions: Yes or No?'
        return 'Question: {0} Options: Yes or No?'.format(self.text)
...
class CheckboxQuestion(MultipleChoiceQuestion):
    ...
    def validate_answer(self, answer: Answer) -> bool:
        ...
        if not isinstance(ans, list) or not ans:
            return False
        for answer_ in ans:
            if ans.count(answer_) != 1 or answer_ not in self._options:
                return False
        return True
...
class Answer:
    ...
    def __eq__(self, other: Answer) -> bool:
        """Return True iff self.content is same as other.content"""
        return self.content == other.content

    def is_valid(self, question: Question) -> bool:
        """Return True iff self.content is a valid answer to <question>"""
        # self: not self.content because validate_answer takes in <Answer>
...
class Survey:
    ...
    def __str__(self) -> str:
        ...
        """
        questions = ''
        survey = self._questions
        keys = []
        values = []
        # must do this or we get 'dict_keys'/'dict_values' object
        # is not subscriptable error
        for i in survey:
            keys.append(i)
            values.append(survey[i])
        # keys = survey.keys()
        ...
        for i in range(len(keys)):
            ...
            q_ans = values[i].text
            # if i == len(keys):
            questions += 'Question' + str(q_id) + ': ' + str(q_ans)
            # questions += 'the question with number' + str(q_id) + 'is' \
            #              + ',' + q_ans + '\n'
        return questions
    ...
    def score_students(self, students: List[Student]) -> float:
        ...
        for q in questions:
            ...
            for student in students:
                answer = student.get_answer(q)
                answers.append(answer)
            # try except not inside 2nd for
            try:
                c = self._get_criterion(q).score_answers(q, answers)
                w = self._get_weight(q)
                score += c * w
