...
class HomogeneousCriterion(Criterion):
    ...
    def score_answers(self, question: Question, answers: List[Answer]) -> float:
        ...
        length = len(answers)
        if length == 1:
            # isinstance(only_ans.content, int) excluded
            if question.validate_answer(answers[0]):
                return 1.0
            else:
        ...
        for i in range(length):
            for j in range(i + 1, length):
                ...
                if question.validate_answer(answers[i]) \
                        and question.validate_answer(answers[j]):
                    sim += question.get_similarity(answers[i], answers[j])
                else:
                    raise InvalidAnswerError
        return sim / total
...
class LonelyMemberCriterion(Criterion):
    ...
    def score_answers(self, question: Question, answers: List[Answer]) -> float:
        ...
        acc = 0
        for answer in answers:
            # write if if, not if else or if elif b/c no pyta error
            if not question.validate_answer(answer):
                raise InvalidAnswerError
